...
import { tambahMenu, uploadMenu } from "../../actions/MenusAction";
...
class TambahMenu extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { uploadMenuResult, tambahMenuResult } = this.props;

    if (uploadMenuResult && prevProps.uploadMenuResult !== uploadMenuResult) {
      this.setState({
        [uploadMenuResult.imageToDB]: uploadMenuResult.image,
      });

      swal("Sukses", "Gambar Berhasil di Upload", "success");
    }

    if (tambahMenuResult && prevProps.tambahMenuResult !== tambahMenuResult) {
      swal("Sukses", "Tambah Menu Sukses Dibuat", "success");
  ...
  handleSubmit = (event) => {
    const { harga, nama, kategori, keterangan, jenis, imageToDB1 } = this.state;

    event.preventDefault();

    if (nama && kategori && keterangan && harga && jenis && imageToDB1) {
      //action
      this.props.dispatch(tambahMenu(this.state));
    } else {
  ...
  render() {
    ...
    const { getListKategoriResult, tambahMenuLoading } = this.props;
    ...
    return (
      <div className="content">
        ...
        <Row>
          <Col>
            <Card>
              <CardHeader tag="h4">Tambah Menu</CardHeader>
              <CardBody>
                <form onSubmit={(event) => this.handleSubmit(event)}>
                  <Row>
                    <Col md={6}>
                      <Row>
                        <Col>
                          <img src={image1} width="300" alt="Foto Jersey " />
                          <FormGroup>
...
const mapStateToProps = (state) => ({
  ...
  tambahMenuLoading: state.MenuReducer.tambahMenuLoading,
  tambahMenuResult: state.MenuReducer.tambahMenuResult,
  tambahMenuError: state.MenuReducer.tambahMenuError,
});

export default connect(mapStateToProps, null)(TambahMenu);
